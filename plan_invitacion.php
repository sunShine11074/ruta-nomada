...
if (!preg_match('/^[a-f0-9]{64}$/', $token)) {
    ...
    if (!$inv) {
        ...
        $db->beginTransaction();
        try {
            // Si ya es miembro no se duplica ni se degrada su rol
            $chk = $db->prepare('SELECT id FROM plan_miembros WHERE plan_id = ? AND usuario_id = ? LIMIT 1');
            $chk->execute([(int)$inv['plan_id'], $userId]);
            if (!$chk->fetch()) {
                $db->prepare('INSERT INTO plan_miembros (plan_id, usuario_id, rol) VALUES (?,?,?)')
                   ->execute([(int)$inv['plan_id'], $userId, $inv['rol']]);
            }
            $db->prepare('UPDATE plan_invitaciones SET usada = 1 WHERE id = ?')->execute([(int)$inv['id']]);
            $db->commit();
            header('Location: plan.php?id=' . (int)$inv['plan_id']);
            exit;
        } catch (Throwable $e) {
            $db->rollBack();
